...
void DriverNode::loop() {
  ...
  if (brightness != brightnessSaved || layerP.maxPower != maxPowerSaved) {
    // Use FastLED for setMaxPowerInMilliWatts stuff, don't use if more then 8096 LEDs, decent power is assumed then!
    uint8_t correctedBrightness = layerP.lights.header.nrOfLights > 8096 ? brightness : calculate_max_brightness_for_power_mW((CRGB*)&layerP.lights.channelsD, MIN(layerP.lights.header.nrOfLights, UINT16_MAX), brightness, layerP.maxPower * 1000);  // calculate_max_brightness_for_power_mW supports max 65K LEDs
